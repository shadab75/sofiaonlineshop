...
{
    function send($amounts,$description,$addressId)
    {
        $data = array(
    ...
    public function verify($authority,$amount)
    {
        ...
        if ($err) {
            ...
            if ($result['Status'] == 100) {
                ...
                \Cart::clear();
                return ['success'=>'تراکنش موفق آمیز بود .شماره پیگیری تراکنش:'.$result['RefID']];
            } else {
                return ['error'=>'تراکنش ناموفق بود .وضعیت:'.$result['Status']];
            }
        }
